...
import { app, FOOTER_HTML, GUEST_STORAGE_KEY, currentUser, setCurrentUser, globalListenersAttached, markGlobalListenersAttached, attachNavToggle, } from '../modules/state';
...
import { getGuestParcels, saveGuestParcels } from '../modules/guest';
import { getCurrentUser } from '../modules/api/auth';
import { fetchParcels, updateParcelDb, emptyTrashDb, deleteParcelDb, insertParcelAfterIndex, } from '../modules/api/parcels';
 ...
 */
function positionParcelIcons() {
    document.querySelectorAll('.icone-colis-wrapper').forEach(icon => {
 ...
 */
function triggerParcelAnimation(id) {
    const grid = document.getElementById('parcelsGrid');
    if (!grid)
        return;
    const card = grid.querySelector(`.parcel-card[data-id="${id}"]`);
    if (!card)
        return;
    const svg = card.querySelector('.rotate-3d');
    if (!svg)
        return;
    svg.classList.remove('rotate-3d--active');
    void svg.offsetWidth; // Force reflow pour relancer l'animation
...
const activePoidsAnims = new Map();
 ...
 */
function attachPoidsThemeObserver() {
    if (poidsThemeObserverAttached)
        return;
    poidsThemeObserverAttached = true;
    new MutationObserver(() => initPoidsAnimations()).observe(document.documentElement, { attributes: true, attributeFilter: ['class'] });
}
 ...
 */
function computePoidsPoints(poidsReel, poidsVol) {
...
function drawPoidsFrame(ctx, pts, scale, col, pr, pv) {
    const cx = PA_SIZE / 2;
    ...
    ctx.fillStyle = col.cF;
    ctx.fill();
    ctx.globalAlpha = 1;
    ctx.strokeStyle = col.cS;
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    ctx.closePath();
    ctx.globalAlpha = col.tAlpha;
    ctx.fillStyle = col.tF;
    ctx.fill();
    ctx.globalAlpha = 1;
    ctx.strokeStyle = col.tS;
    ctx.lineWidth = 2;
    ctx.stroke();
...
function animatePoidsComparaison(canvas, pr, pv) {
...
function initPoidsAnimations() {
    document.querySelectorAll('.anim-poids-wrapper').forEach(w => {
        const canvas = w.querySelector('.anim-poids-canvas');
        if (!canvas)
            return;
        const ctx = canvas.getContext('2d');
        if (!ctx)
...
function positionPoidsAnimations() {
    document.querySelectorAll('.anim-poids-wrapper').forEach(anim => {
        anim.style.right = `-${GUTTER_CENTER_PX}px`;
    });
}
/** Déclenche l'animation comparaison poids sur le colis spécifié */
function triggerPoidsAnimation(id) {
    const grid = document.getElementById('parcelsGrid');
    if (!grid)
        return;
    const card = grid.querySelector(`.parcel-card[data-id="${id}"]`);
    if (!card)
        return;
    const w = card.querySelector('.anim-poids-wrapper');
    if (!w)
        return;
    const canvas = w.querySelector('.anim-poids-canvas');
    if (!canvas)
    ...
    const pv = parseFloat(w.dataset.poidsVol || '0');
    if (pr > 0 && pv > 0)
        animatePoidsComparaison(canvas, pr, pv);
}
// ── Render ───────────────────────────────────────────────────────────────────
...
// ── Grid ─────────────────────────────────────────────────────────────────────
export function renderParcelsGrid(parcels) {
    const grid = document.getElementById('parcelsGrid');
    ...
    grid.innerHTML = parcels.map((p, i) => createParcelCard(p, i)).join('');
...
function createParcelCard(parcel, index) {
    ...
    const displayName = parcel.name || `Colis #${index + 1}`;
...
function renderPanel(containerId, parcels) {
    const container = document.getElementById(containerId);
    if (!container)
        return;
    if (parcels.length === 0) {
        container.innerHTML = '<p style="text-align:center;color:#888;">Aucun colis</p>';
        return;
    }
    const SVG_EDIT = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>`;
...
// ── Duplicate ────────────────────────────────────────────────────────────────
async function duplicateParcel(sourceId, sourceName) {
    ...
    showRenameModal(newParcel.id, `${baseName} (copie)`, async () => {
        const ps = await fetchParcels('active');
        renderParcelsGrid(ps);
        loadSidePanels();
    });
}
// ── Recap report ─────────────────────────────────────────────────────────────
function renderRecapReport() {
    ...
    if (!grid || !recapEl)
        return;
    const cards = grid.querySelectorAll('.parcel-card');
    if (cards.length === 0) {
        recapEl.innerHTML = '';
        return;
    }
    ...
    cards.forEach((card, idx) => {
        ...
        const width = rv('width');
        const height = rv('height');
        const realWeight = rv('real_weight');
        const invalidFields = [];
        if (!length)
        ...
        if (!realWeight)
            invalidFields.push('real_weight');
        const invalid = invalidFields.length > 0;
        ['length', 'width', 'height', 'real_weight'].forEach(f => {
            card.querySelector(`input[data-field="${f}"]`)
    ...
    const labels = { length: 'Longueur', width: 'Largeur', height: 'Hauteur', real_weight: 'Poids réel' };
    const rowsHtml = summaries.map(s => s.invalid
       ...
       <td class="recap-cell recap-cell--billed">${s.billedWeight.toFixed(2)} kg</td></tr>`).join('');
    recapEl.innerHTML = `
    ...
    document.getElementById('recapCloseBtn')?.addEventListener('click', () => {
        recapEl.innerHTML = '';
        grid.querySelectorAll('.parcel-card').forEach(c => c.classList.remove('recap-invalid'));
        grid.querySelectorAll('input[data-field]').forEach(i => i.classList.remove('error'));
    });
    recapEl.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
// ── Guest actions ────────────────────────────────────────────────────────────
function handleGuestAction(id, action, card) {
    ...
    if (action === 'calculate' && card) {
        ...
        inputs.forEach(inp => {
            ...
            if (bad) {
                hasInvalid = true;
                inp.classList.remove('flash-error');
                void inp.offsetWidth;
                inp.classList.add('flash-error');
...
function setupEventListeners() {
    ...
    document.getElementById('addParcelBtn')?.addEventListener('click', async () => {
        if (currentUser) {
            ...
            const lastPos = existing.length > 0 ? (existing[existing.length - 1].position ?? existing.length * 10) : 0;
            const insertData = { user_id: currentUser, status: 'active' };
            const { data, error } = await supabase
                .from('parcels')
                .insert({ ...insertData, position: lastPos + 10 })
                .select().single();
            if (!error && data) {
                const ps = await fetchParcels('active');
                renderParcelsGrid(ps);
            ...
            if (guestParcels.length >= 3) {
                const notice = document.querySelector('.guest-notice');
                if (notice) {
    ...
    document.__syncNavActive = syncNavActive;
    ...
    document.querySelector('a[data-view="home"]')?.addEventListener('click', e => {
        e.preventDefault();
        import('./router').then(r => r.navigateTo('home'));
    });
    ...
    if (!globalListenersAttached) {
        ...
        document.addEventListener('input', e => {
            ...
            if (!bad) {
                const card = input.closest('.parcel-card');
                if (card) {
                    const still = Array.from(card.querySelectorAll('input[data-field]'))
        ...
        document.addEventListener('click', async (e) => {
            ...
            const card = target.closest('.parcel-card');
            const panelItem = target.closest('.panel-item');
            if (!card && !panelItem)
                return;
            ...
            if (!id || !action)
                return;
            if (!currentUser) {
                handleGuestAction(id, action, card);
                return;
            }
            switch (action) {
                case 'delete':
                    await updateParcelDb(id, { status: 'deleted' });
                    break;
                case 'archive':
                    await updateParcelDb(id, { status: 'archived' });
                    break;
                case 'restore':
                    await updateParcelDb(id, { status: 'active' });
                    break;
                case 'archive-delete':
                    await updateParcelDb(id, { status: 'deleted' });
                    break;
                case 'permanent-delete':
                    await deleteParcelDb(id);
                    break;
                case 'rename': {
...
function genererSvgColisDynamique(longueur, largeur, hauteur) {
